...
@section('title', 'Product Sales Reports')
...
@section('content')
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
            <div class="col-lg-12">
                <form method="GET" action="{{ url('/admin/product-sales-report') }}">
                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                        <div class="table-responsive">
                            <table id="productSalesTable" class="display table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        <th>Product Name</th>
                                        <th>Total Sales (<span class="icon-aed">{{ getSetting('currency') }}</span>)</th>
                                        <th>VAT 5% (<span class="icon-aed">{{ getSetting('currency') }}</span>)</th>
                                        <th>Total Quantity Sold</th>
                                        <th>Avg Sale Price (<span class="icon-aed">{{ getSetting('currency') }}</span>)</th>
                                    </tr>
                                </thead>
                                <tfoot>
                                    <tr>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            let table = $('#productSalesTable').DataTable({
                processing: true,
                serverSide: true,
                dom: 'Bfrtip',
                buttons: [{
                        ...
                        action: function(e, dt, button, config) {
                            let fullDataUrl =
                                '{{ url('/admin/api/product-sales-report') }}?date_filter=' + $('#date_filter').val() +
                                '&from_date=' + $('#from_date').val() +
                                '&to_date=' + $('#to_date').val() + '&export_all=true';
                            window.location.href = fullDataUrl + '&format=csv';
                        }
                    },
                    {
                        extend: 'excel',
                        ...
                        action: function(e, dt, button, config) {
                            let fullDataUrl =
                                ...
                                '&from_date=' + $('#from_date').val() +
                                '&to_date=' + $('#to_date').val() + '&export_all=true';
                            window.location.href = fullDataUrl + '&format=excel';
                        }
                    },
                    {
                        extend: 'pdf',
                        ...
                        action: function(e, dt, button, config) {
                            let fullDataUrl =
                                ...
                                '&from_date=' + $('#from_date').val() +
                                '&to_date=' + $('#to_date').val() + '&export_all=true';
                            window.location.href = fullDataUrl + '&format=pdf';
                        }
                    },
                    {
                        extend: 'print',
                ...
                ajax: {
                    url: '{{ url('/admin/api/product-sales-report') }}',
                    data: function(d) {
                ...
                footerCallback: function(row, data, start, end, display) {
                    ...
                    console.log('Product Sales AJAX Response:', json);
                    ...
                    try {
                        if (json && json.grandTotals) {
                            ...
                            grandTotalRow.find('th').eq(2).html(grandTotals.vat || '0.00');
                            grandTotalRow.find('th').eq(3).html(grandTotals.total_quantity || '0');
                            ...
                            grandTotalRow.find('th').eq(2).html('0.00');
                            grandTotalRow.find('th').eq(3).html('0');
                            grandTotalRow.find('th').eq(4).html('');
