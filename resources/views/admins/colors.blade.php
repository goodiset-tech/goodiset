              ...
              <form role="form" class="form-inline" autocomplete="off" method="post">
                  @csrf
                        <div class="form-group" style="display: flex;flex-direction: column;">
                          <label for="exampleInputEmail2">Color Name or Code</label>
      ...
      <div class="ibox float-e-margins">
          ...
          <tbody>
            ...
            @foreach ($categories as $item)
                <tr>
                  <td>{{$sr++}}</td>
                  <td><span style="background: {{$item->name}};padding: 10px 20px;position: relative;top: 10px;"></span></td>
                  <td>{{\Carbon\Carbon::parse($item->created_at)->diffForHumans()}}</td>
                  <td>
                    <a href="{{route('admins.colors')}}/{{$item->id}}" class="btn btn-warning">Edit</a>
                    <a href="javascript:void(0)" data-href="{{route('admins.colors')}}/{{$item->id}}/{{'delete'}}"  class="btn btn-danger delete_record">Delete</a>
                  </td>
                </tr>
            @endforeach
          </tbody>
