...
@section('content')
                    ...
                    <div class="content1">
                        ...
                        <div class="column"
                            ...
                            <div class="summary_card">
                                ...
                                <hr>
                                <div class="sutotal">
                                    <h2>{{ __('checkout.totals.subtotal') }} </h2> <span></span><span
                                        class="price cartTotal1" id="cartTotal1">{{ getSetting('currency') }}
                                        {{ Session::get('cart')['amount'] }}</span>
                                </div>
                                <div class="shipping">
                                    <h2>{{ __('checkout.totals.shipping') }} </h2> <span></span><span
                                        class="price shipping_fee"></span>
                                </div>
                                <div class="shipping">
                                    @if (getSetting('tax_value') > 0)
                                        <h2>{{ __('checkout.totals.vat_with_rate', ['tax' => getSetting('tax_value')]) }}
                                        </h2> <span></span><span class="vat">{{ getSetting('currency') }}
                                            {{ (Session::get('cart')['amount'] * getSetting('tax_value')) / 100 }} </span>
                                    @else
                                        <h2>{{ __('checkout.totals.vat') }} </h2> <span></span><span
                                            class="vat">{{ __('checkout.totals.vat_included') }} </span>
                                    @endif
                                </div>
                                <div class="shipping">
                                    <h2>{{ __('checkout.totals.discount') }} </h2> <span></span><span
                                        class="price">{{ getSetting('currency') }}
                        ...
                        <form action="" id="paymentForm" class="shipping_details">
                            ...
                            <div class="column"
                                ...
                                <div class="free-shipping summary_card">
                                    <div class="min-order"
                                        ...
                                        <div class="content"
                                            ...
                                            <span class="min_shipping icon-aed"></span>
                                            <span class="max_free_shipping icon-aed"></span>
                                        </div>
                                ...
                                <div class="summary_card">
                                    ...
                                    <div class="sutotal ">
                                        <h2>{{ __('checkout.totals.subtotal') }} </h2> <span></span><span
                                            class="price cartTotal1 icon-aed"
                                            id="cartTotal1">{{ getSetting('currency') }}
                                            {{ Session::get('cart')['amount'] }}</span>
                                    </div>
                                    <div class="shipping ">
                                        <h2>{{ __('checkout.totals.shipping') }} </h2> <span></span><span
                                            class="price shipping_fee icon-aed"> 0.00</span>
                                    </div>
                                    <div class="shipping">
                                        @if (getSetting('tax_value') > 0)
                                            <h2>{{ __('checkout.totals.vat_with_rate', ['tax' => getSetting('tax_value')]) }}
                                    ...
                                    <div class="shipping">
                                        <h2>{{ __('checkout.totals.discount') }} </h2> <span></span><span
                                            class="price icon-aed">{{ getSetting('currency') }}
                                            {{ Session::get('cart')['discount'] ?? 0 }}</span>
                                    </div>
                                    <hr>
                                    <div class="total">
                                        <h2>{{ __('checkout.totals.total') }}</h2>
                                        <span></span><span class="price cartTotal icon-aed"
                                            id="cartTotal">{{ getSetting('currency') }}
                                            {{ Session::get('cart')['amount'] + (Session::get('cart')['amount'] * getSetting('tax_value')) / 100 }}</span>
                                    </div>
                                    <p id="show_meaaage1" style="display: none;color: #e92827;"> </p>
                                    <button id="next_btn" class="next checkout"
                                        type="button">{{ __('checkout.actions.proceed_to_payment') }}</button>
                                    <button class="continue_shopping">
                                        <a href="{{ url('/') }}"> {{ __('checkout.actions.continue_shopping') }}
                                        </a>
                                    </button>
                                    <div class="cards">
                                        ...
                                        <svg width="43" height="29" viewBox="0 0 43 29" fill="none"
                                            ...
                                            <defs>
                                                <pattern id="pattern0_300_17073" patternContentUnits="objectBoundingBox"
                                                    width="1" height="1">
                                                    <use xlink:href="#image0_300_17073"
                                                        transform="scale(0.0025 0.00628931)"></use>
                                                </pattern>
                                                <image id="image0_300_17073" width="400" height="159"
                                                    preserveAspectRatio="none"
                    ...
                    <div class="content1">
                        <form id="paymentForm1" class="shipping_details">
                            <div class="payment_method">
                                @php
                                    $cridit = PaymentMethod::where('method_name', 'Ngenius')->first();
                                @endphp
                                ...
                                @endif
                                @php
                                    $cridit = PaymentMethod::where('method_name', 'Stripe')->first();
                                @endphp
                                @if ($cridit->is_enabled == 1)
                                    <div class="debit_card payment">
                                    ...
                                    <div class="card_details" style="max-width:418px;">
                                        <div class="fields" style="gap:14px;">
                                            ...
                                            <div class="field">
                                                <label style="display:unset;"
                                                    for="card_holder_name">{{ __('checkout.card.holder_name') }}
                                                    <span style="color: #e92827">*</span></label>
                                        ...
                                        <div id="express-checkout-button">
                                ...
                                @endif
                                @php
                                    $Apple_pay = PaymentMethod::where('method_name', 'Apple Pay')->first();
                                @endphp
                                ...
                                @endif
                                @php
                                    $Google_pay = PaymentMethod::where('method_name', 'Google Pay')->first();
                                @endphp
                                ...
                                @endif
                                @php
                                    $Cod = PaymentMethod::where('method_name', 'Cod')->first();
                                @endphp
                                @if ($Cod->is_enabled == 1)
                                    <div class="cash_onDelivery payment">
                                        ...
                                        <label for="cash_onDelivery">
                                            <h1><i
                                                    class="fa-solid fa-hand-holding-dollar"></i>{{ __('checkout.payment.cod.name') }}
                                            </h1>
                        ...
                        <div class="summary_card">
                            ...
                            <div class="shipping">
                                <h2>{{ __('checkout.totals.shipping') }} </h2> <span></span><span
                                    class="price shipping_fee">{{ getSetting('currency') }} 0.00
                                </span>
                            </div>
                            <div class="shipping">
                                @if (getSetting('tax_value') > 0)
                            ...
                            <div class="total">
                                <h2>{{ __('checkout.totals.total') }}</h2>
                                <span></span><span class="price cartTotal finaltotal"
                                    id="cartTotal">{{ getSetting('currency') }}
                                    {{ Session::get('cart')['amount'] + (Session::get('cart')['amount'] * getSetting('tax_value')) / 100 }}</span>
                            </div>
                            <button class="next checkout" id="submitOrder" type="button">
                                {{ __('checkout.actions.submit_order') }} <i id="spinner"
                                    class="fa fa-spinner fa-spin" style="display:none"></i></button>
                            <div class="cards">
                                <!-- (payment card svgs unchanged) -->
                                ... (SVGs)
                            </div>
                        </div>
                    </div>
    ...
    <!-- CONTAINER END -->
    <div class="trending_section" style="background-color: #FFFAFA; padding-bottom:0px; display:none;">
        <h3 class="trending-heading">You might also like</h3>
                    ...
                    @foreach ($pro as $v)
                        <div class="card-container" style="background: white;">
                            ...
                            <a href="<?php echo url('/'); ?>/product/{{ $v->slug }}">
                                <div class="image">
                                    ...
                                    <div class="product_slider" style="z-index: 1;">
                                        <img src="{{ asset($v->thumb) }}" alt="Background Image 1"
                                            class="product_slider_image  active">
                                        <img src="{{ asset($v->image_one) }}" alt="Background Image 2"
                            ...
                            <div class="content">
                                ...
                                @php
                                    $data = Rating::where('pid', $v->id)->where('status', 1)->sum('rate');
                                    $count = Rating::where(['pid' => $v->id])
                                        ->where('status', 1)
                                        ->count();
                                    if ($count && $data) {
                                        $rate = $data / $count;
                                ...
                                <div class="stars rating_stars">
                                    @for ($i = 1; $i <= 5; $i++)
                                        @if ($rate >= $i)
                                            <i class="fa fa-star checked"></i> <!-- Fully filled star -->
                                        @elseif ($rate > $i - 1)
                                ...
                                <p class="rats" style="text-align: left !important;">
                                    <?php
                                    ...
                                    ?>
                                    {{ $v->discount_price }} {{ getSetting('currency') }} @if ($selling_price > 0)
                                        <del class="ml">{{ $v->selling_price }} {{ getSetting('currency') }}</del>
                                        <span class="red-text">({{ $discount_percentage }}% Off)</span>
                                    @endif
                                </p>
                            </div>
                            <div class="quantity quantity-controls quantity_btn_box{{ $v->id }}"
                                id="quantity_btn_box"
                                style="{{ App\Helpers\Cart::has_pro($v->id) ? '' : 'display:none;' }}">
                                <div class="button_spin_overlay" id="button_loader{{ $v->id }}"
                                    style="display: none">
                                    <div class="loader_dots"></div>
                                </div>
                                ...
                                @php
                                    ...
                                    $quantity = $quantity ?? 1;
                                @endphp
                                <i class="fa-solid fa-minus minus_quantity minus" productId="{{ $v->id }}"
                                    productprice="{{ $v->discount_price }}" id="minus"></i>
                                <input type="hidden" class="form-control form-control-sm bg-secondary text-center"
                                    id="spec{{ $v->id }}" id="qty" name="qty" value="1">
                                <span class="quantity_cart" id="quantity{{ $v->id }}">
                                    @if ($v->format == 1)
                                        {{ $quantity * 100 }} g
                                    @else
                                        {{ $quantity * 1 }}
                                ...
                                <i class="fa-solid fa-plus add_quantity plus" productId="{{ $v->id }}"
                                    productprice="{{ $v->discount_price }}" id="plus"></i>
                            </div>
                            <div class="button add-to-cart{{ $v->id }}"
                                style="{{ App\Helpers\Cart::has_pro($v->id) ? 'display:none;' : '' }}">
                                <button class="add-to-cart " <?php if ($v->product_quantity < 1) {
                                    echo 'disabled';
                                } ?>
    ...
    <script>
        ...
        const userEmail = '{{ session()->has('user') ? session('user')['email'] : session('cart')['email'] ?? '' }}';
        const userPhone = '{{ session()->has('user') ? session('user')['phone'] : session('cart')['phone'] ?? '' }}';
        ...
        const uuidC1 = uuidC1Match ? decodeURIComponent(uuidC1Match[1]) : null;
        ...
        try {
            if (window.dataLayer) {
                if (productDetail.length > 0) {
                    ...
                    console.log("DataLayer pushed for checkout:", window.dataLayer);
                }
            } else {
                console.warn("dataLayer is not defined.");
            }
        } catch (checkoutError) {
            console.error("Error pushing to dataLayer for checkout:", checkoutError);
        }

        try {
            if (productDetail.length > 0 && typeof snaptr === "function") {
                const payload = {
                    price: cartAmount, // total value
                    currency: "AED",
                    item_ids: itemIds, // IDs from your items
                    number_items: numberItems, // total quantity in cart
                    payment_info_available: 0 // at START_CHECKOUT this is usually 0/false
                };

                // Only include optional fields if we actually have values
                if (uuidC1) payload.uuid_c1 = uuidC1;
                if (userEmail) payload.user_email = userEmail;
                if (userPhone) payload.user_phone_number = userPhone;

                // If no IDs were found, it's safer to omit item_ids than send placeholders
                if (!payload.item_ids.length) delete payload.item_ids;

                snaptr('track', 'START_CHECKOUT', payload);
                console.log("Snap START_CHECKOUT sent:", payload);
            }
        } catch (error) {
            console.error("Error sending snaptr START_CHECKOUT event:", error);
        }
    </script>
    <script src="{{ asset('') }}front/checkout.js"></script>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateShippingUI(rules) {
                ...
                // Update UI elements
                $('#minOrderMessage').html(`Minimum Order:<span class="icon-aed"> ${minOrder} ${currency} </span>`);
                $('.min_shipping').html(`<span class="icon-aed">${minOrder} ${currency}</span>`);
                $('.max_free_shipping').html(`<span class="icon-aed">${freeShippingThreshold} ${currency}</span>`);
                ...
                if (cartTotal < minOrder) {
                    ...
                    $('#show_meaaage').html(`Minimum Order:<span class="icon-aed"> ${minOrder} ${currency} </span>`)
                        .show();
                    $('#next_btn').prop('disabled', true);
                    $('#show_meaaage1').html(
                            `Minimum Order for ${name} : <span class="icon-aed">${minOrder} ${currency}</span>`)
                        .show();
                } else {
                ...
                if (cartTotal >= freeShippingThreshold) {
                    ...
                    $('.shipping_fee').html(`<span class="icon-aed"> ${currency} 0.00</span>`);
                    $('.shipping_value').html(`<span class="icon-aed">${currency} 0.00</span>`);
                    ...
                    $('#shippingCost').text(
                        `Shipping Cost:<span class="icon-aed"> ${currency} ${shippingCost} </span>`);
                    $('.shipping_fee').html(`<span class="icon-aed"> ${currency} ${shippingCost}</span>`);

                    $('.shipping_value').html(`<span class="icon-aed">${currency} ${shippingCost}</span>`);
            ...
            function handlePaymentMethodChange() {
                ...
                const submitButton = document.getElementById('submitOrder');
                document.querySelector('.card_details').style.display = selectedMethod === 'stripe' ? 'block' :
                    'none';
                ...
                if (selectedMethod === 'stripe') {
                    ...
                    document.getElementById("express-checkout-button").style.display = selectedMethod ===
                        "express_checkout" ? "block" : "none";
            ...
            function handlePaymentMethodChange() {
                ...
                if (selectedMethod === 'stripe') {
                    ...
                    googlePayButton.style.display = selectedMethod === 'google_pay' ? 'block' : 'none';
                    submitButton.disabled = false;
                }
            }
            ...
            document.getElementById('submitOrder').addEventListener('click', async (e) => {
                ...
                if (selectedMethod === 'stripe') {
                    ...
                    const {
                        token,
                        error
                    } = await stripe.createToken(window.cardNumber); // ✅ Pass the correct card element
    ...
    <script>
        ...
        function updateShippingUI(rules) {
            ...
            $('.max_free_shipping').html(`<span class="icon-aed">${currency} ${freeShippingThreshold} </span>`);
            ...
            if (cartTotal < minOrder) {
                ...
                $('#next-step').prop('disabled', true);
                $('#show_meaaage').html(`Minimum Order: <span class="icon-aed">${currency} ${minOrder} </span>`).show();
                $('#next_btn1').prop('disabled', true);
                $('#show_meaaage1').html(
                    `Minimum Order for ${name} : <span class="icon-aed">${currency} ${minOrder} </span>`).show();
            } else {
                $('#next-step').prop('disabled', false);
                $('#show_meaaage').hide();
        ...
        const MSG_REQUIRED = @json(__('checkout.validation_required'));
        const MSG_email = @json(__('checkout.email_required'));
        const MSG_phone = @json(__('checkout.phone_required'));
        document.addEventListener("DOMContentLoaded", function() {
            fetchSessionData();
            ...
            function validateForm() {
                const form = document.querySelector(".shipping_details");
                ...
                requiredFields.forEach((field) => {
                    ...
                    if (!value) {
                        ...
                        showError(field, MSG_REQUIRED);
                        if (!firstInvalidField) firstInvalidField = field;
                    } else if (field.type === "email" && !isValidEmail(value)) {
                        isValid = false;
                        field.style.border = "2px solid red";
                        showError(field, MSG_email);
                        if (!firstInvalidField) firstInvalidField = field;
                    } else if (field.type === "tel" && !isValidPhone(value)) {
                        isValid = false;
                        field.style.border = "2px solid red";
                        showError(field, MSG_phone);
                        if (!firstInvalidField) firstInvalidField = field;
                    } else {
                        field.style.border = "";
                        hideError(field);
            ...
            function isValidPhone(phone) {
                const onlyAllowed = phone.replace(/[^\d+()\s.-]/g, '');
                const plusCount = (onlyAllowed.match(/\+/g) || []).length;
                if (plusCount > 1) return false;
                if (plusCount === 1 && onlyAllowed.trim()[0] !== '+') return false;
                const digitsOnly = onlyAllowed.replace(/[^\d]/g, '');
                if (digitsOnly.length < 7 || digitsOnly.length > 15) return false;
                if (onlyAllowed.trim().startsWith('+') && digitsOnly[0] === '0') return false;
                return true;
            }
            ...
            try {
                ...
                if (data.success) {
                    if (data.payment_method === 'ngenius') {
                        ...
                        window.location.href = ngeniusData.paymentLink || "{{ url('/thanks') }}/" + data.order_no;
                    } else {
                        $('#submitOrder').prop('disabled', false);
                        $('#spinner').hide();
                        $('#loader_container').hide();
        ...
        async function storeFormData() {
            const finalTotal = parseFloat(document.querySelector('.finaltotal').textContent.replace(/[^0-9.]/g, ''));
            let formData = {
            ...
            try {
                const response = await fetch("{{ route('store.checkout.session') }}", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
